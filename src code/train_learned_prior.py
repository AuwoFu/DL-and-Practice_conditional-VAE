...
from utils import init_weights, kl_criterion_LP,  finn_eval_seq, pred_with_LP, rec,plot_seq
...
def train(x, cond, modules, optimizer, kl_anneal, args):
    ...
    if use_teacher_forcing:
        for i in range(1, n_frame):
            ...
            if i<=args.n_past:
                h,skip=h_seq[i-1] # h2,skip=modules['encoder'](x[:,i-1])
            else:
                h=h_seq[i-1][0]
        ...
        for i in range(1, args.n_past + args.n_future):
            ...
            if i<=args.n_past:
                h=h_seq[i-1][0] # feed 2 ground truth
                skip=h_seq[i-1][1]
            else:
...
class kl_annealing():
    def __init__(self, args):
        ...
        if self.useCyclical:
            #self.period=args.niter/self.CyclicalCycle # each cycle length
        ...
        print(f'KL step={self.step}, KL period={self.period}')
    ...
    def update(self,current_epoch):   
        ...
        if self.beta>self.max:
            self.beta=self.max
        #print(f'KL={self.beta} after iter {current_epoch}')
        return
...
def main():
    ...
    for epoch in range(start_epoch, start_epoch + niter):
        ...
        if (epoch)%2==0 and epoch >= args.tfr_start_decay_epoch:
        ...
        if epoch % 5 == 0:
            ...
            for _ in range(len(validate_data) // args.batch_size):
                ...
                pred_seq = pred_with_LP(validate_seq, validate_cond, modules, args, device)
